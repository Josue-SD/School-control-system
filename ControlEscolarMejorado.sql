...
INSERT INTO ALUMNO_CALIFICACION VALUES (NULL, 1, 11, 8.5);
DELETE FROM ALUMNO_CALIFICACION WHERE idCalificacion = 11;
UPDATE ALUMNO_CALIFICACION SET calificacion = "mexicana" WHERE idCalificacion = 1;
SELECT * FROM ALUMNO_CALIFICACION;
...
SELECT * FROM LOGIN ORDER BY numCuenta;
...
SELECT * FROM Grupos_Alumnos ORDER BY idGrupo;
SELECT * FROM ALUMNO_CALIFICACION ORDER BY idCalificacion;



--CONSULTAS UTILES

SELECT nombreCompleto, genero, nacionalidad, CURP, fechaNac FROM alumno WHERE numCuenta = 13;
...
INSERT INTO REINSCRIPCION VALUES(2005,21,1);
...
SELECT ALUMNO.nombreCompleto
FROM REINSCRIPCION
JOIN ALUMNO ON REINSCRIPCION.idAlumno = ALUMNO.numCuenta;

SELECT ALUMNO.nombreCompleto, ALUMNO_CALIFICACION.calificacion
FROM REINSCRIPCION
JOIN ALUMNO ON REINSCRIPCION.idAlumno = ALUMNO.numCuenta
JOIN ALUMNO_CALIFICACION ON REINSCRIPCION.idAlumno = ALUMNO_CALIFICACION.idAlumno;

SELECT ALUMNO.nombreCompleto, ALUMNO_CALIFICACION.calificacion, PERIODO.fechaFin
FROM REINSCRIPCION
JOIN ALUMNO ON REINSCRIPCION.idAlumno = ALUMNO.numCuenta
JOIN ALUMNO_CALIFICACION ON REINSCRIPCION.idAlumno = ALUMNO_CALIFICACION.idAlumno
JOIN PERIODO ON REINSCRIPCION.idPeriodo = PERIODO.idPeriodo;


SELECT tipoUsuario FROM LOGIN WHERE numCuenta = %s AND contrasena = %s;

SELECT nombreCompleto, genero, nacionalidad, CURP, fechaNac FROM alumno WHERE numCuenta = 1;

UPDATE alumno SET nombreCompleto = "Elena María Venadero Bastida", genero = "M", nacionalidad = "mexicana", CURP =  "SADJ030113HMCNMSA6", fechaNac = "2003-12-12" WHERE numCuenta = 1;

DELETE FROM LOGIN WHERE numCuenta = 11;

DELETE FROM alumno WHERE numCuenta = 11;

SELECT * FROM ALUMNO WHERE NUMCUENTA = 15;

SELECT * FROM ALUMNO WHERE curp = "SADJ030113HMCNMSA6";

INSERT INTO PERIODO VALUES (NULL, DEFAULT, DEFAULT, DEFAULT);

INSERT INTO MATERIA
VALUES ('Bases de datos', '2024-06-15', '2024-08-15', '07:00:00', '14:00:00', 'Lunes');
